...
class EncyclopediaDepVC: BaseViewController , UITableViewDelegate , UITableViewDataSource  {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "EncyclopediaDepCell") as! EncyclopediaDepCell
    ...
    @objc private func handelRefresh() {
        ...
        if let api_token = ApiToken.getApiToken() {
            ...
            ApiMethods.GetEncyclopediaMainDep(api_token: api_token, compltion: { (status, error, api_token_status, articles, last_page) in
    ...
    func loadMore ()  {
        ...
        if let api_token = ApiToken.getApiToken() {
            ...
            ApiMethods.GetEncyclopediaMainDep(api_token: api_token, page: self.current_page+1) { (status,api_token_status,error ,articles , last_page) in
                self.activityIndicator.stopAnimating()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        print("Selected")
        print("Single Article" , self.Articles[indexPath.row])
        performSegue(withIdentifier: "EncyclopediaDep", sender: self.Articles[indexPath.row])
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let dist = segue.destination as? EncyclopediaVC {
            if let Item = sender as? EncyclopediaDepModel {
                dist.DepId = Item.id
